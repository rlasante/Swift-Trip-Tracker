...
class TripTests: XCTestCase {
  ...
  func testAddingLocation() {
    ...
    XCTAssert(trip.locations.count == 1)
    XCTAssert(trip.locations[0].coordinate.latitude == lat)
    XCTAssert(trip.locations[0].coordinate.longitude == long)
  }
  ...
  func testAddMultipleLocations() {
    ...
    XCTAssert(trip.locations.count == 2)
    for (index, location) in trip.locations.enumerate() {
